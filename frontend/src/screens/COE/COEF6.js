// CCSF6.js
// Event screen with reminder bars + big gradient calendar + overlapping red square
// Reference preview image (optional): /mnt/data/3e0bc86c-ddb9-4cbc-8cd0-244bc92d8a2b.png
...
export default function COEF6({ navigation }) {
  ...
  return (
    <SafeAreaView style={s.screen}>
      ...
      <View style={s.container}>
        ...
        <View style={s.reminders}>
          <View style={s.reminderRow}>
            <LinearGradient colors={['rgba(0,0,0,0.06)', 'rgba(0,0,0,0.02)']} start={[0,0]} end={[1,0]} style={s.reminderBase}>
              <LinearGradient colors={['#ff9a8b', '#ff6a6a']} start={[0,0]} end={[1,1]} style={s.reminderGradientLeft} />
        ...
        {/* Overlapping red square attached to big card */}
...
const s = StyleSheet.create({
  ...
  weekdayText: { width: (Math.min(340, width - 48) - 24) / 7, textAlign: 'center', fontSize: 12, color: 'rgba(255,255,255,0.85)', fontWeight: '600' },
  ...
  /* overlapping red square (positioned relative to big card) */
  overlapSquare: {
    position: 'absolute',
    right: 12,
    bottom: 112 * -0.0, // unused; kept for compatibility
    // we'll position visually under the big card by using container alignment
    marginTop: 10,
    width: 56,
    height: 56,
