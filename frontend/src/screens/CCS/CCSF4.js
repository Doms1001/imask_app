...
export default function COAF4({ navigation }) {
  const enterAnims = [ useRef(new Animated.Value(0)).current, useRef(new Animated.Value(0)).current, useRef(new Animated.Value(0)).current ];
  const pressScales = [ useRef(new Animated.Value(1)).current, useRef(new Animated.Value(1)).current, useRef(new Animated.Value(1)).current ];
  const glowOpac = [ useRef(new Animated.Value(0)).current, useRef(new Animated.Value(0)).current, useRef(new Animated.Value(0)).current ];
  const tilt = [ useRef(new Animated.ValueXY({ x: 0, y: 0 })).current, useRef(new Animated.ValueXY({ x: 0, y: 0 })).current, useRef(new Animated.ValueXY({ x: 0, y: 0 })).current ];
  const shimmer = useRef(new Animated.Value(-1)).current;

  useEffect(() => {
    const seq = enterAnims.map((a, i) => Animated.timing(a, { toValue: 1, duration: 420, delay: i * 120, useNativeDriver: true }));
  ...
  function makeResponder(i) {
    return PanResponder.create({
      ...
      onPanResponderGrant: () => Animated.spring(pressScales[i], { toValue: 0.95, friction: 7, tension: 200, useNativeDriver: true }).start(),
  ...
  function navSafe(route) { if (navigation?.navigate) navigation.navigate(route); }
        ...
        <View style={m.leftColumn}>
          <Animated.View {...responders[0].panHandlers} style={[m.cardWrapper, cardAnimatedStyle(0)]}>
            <Animated.View style={[m.card, m.cardLarge]}>
              <LinearGradient colors={['#ffd500', '#000000']} start={[0, 0]} end={[1, 1]} style={StyleSheet.absoluteFill} />
...
const s = StyleSheet.create({
  ...
  layerTopRight: { position: 'absolute', right: -40, top: -20, width: 220, height: 220, borderRadius: 18, backgroundColor: '#0b0b0b' },
  layerBottomLeft: { position: 'absolute', left: -60, bottom: -80, width: 260, height: 260, borderRadius: 160, backgroundColor: '#ffd500', opacity: 0.72 },
...
const m = StyleSheet.create({
  ...
  card: {
    width: 160, borderRadius: 18, padding: 12, backgroundColor: '#fff',
    shadowColor: '#000', shadowOpacity: 0.18, shadowOffset: { width: 0, height: 10 }, shadowRadius: 20, elevation: 10, overflow: 'hidden',
  },
  ...
  cardGlow: { position: 'absolute', bottom: -8, width: 160, height: 22, borderRadius: 12, backgroundColor: 'rgba(255,213,0,0.25)' },
