// CBAF4.js — Yellow/Orange Theme (tilt, press, shimmer kept)
...
export default function CBAF4({ navigation }) {
  ...
  useEffect(() => {
    const entranceAnim = Animated.timing(entrance, { toValue: 1, duration: 700, useNativeDriver: true });
    entranceAnim.start();
    ...
    const pulses = glowOpac.map((g, i) =>
      Animated.loop(
        Animated.sequence([
          Animated.timing(g, { toValue: 0.12, duration: 1200 + i * 80, useNativeDriver: true }),
          Animated.timing(g, { toValue: 0.0, duration: 1200 + i * 80, useNativeDriver: true }),
        ])
      )
    );
  ...
  return (
    <SafeAreaView style={s.screen}>
        ...
        <Animated.View {...responders[0].panHandlers} style={[s.topWrapper, cardAnimatedStyle(0)]}>
          ...
          >
            <Animated.View style={[s.topCard]}>
              ...
              <Animated.View style={[s.shimmer, { opacity: 0.28, transform: [{ translateX: shimmerTranslate }, { rotate: '22deg' }] }]} pointerEvents="none" />
      ...
      <BottomPager navigation={navigation} activeIndex={2} targets={['CBAF2', 'CBAF3', 'CBAF4']} />
    </SafeAreaView>
  );
}

/* Styles (kept structure, updated warm colors) */
const s = StyleSheet.create({
  screen: { flex: 1, backgroundColor: '#fff', alignItems: 'center' },
  bg: { ...StyleSheet.absoluteFillObject },

  layerTopRight: { position: 'absolute', right: -40, top: -20, width: 220, height: 150, borderRadius: 150 },
  layerBottomLeft: { position: 'absolute', left: -90, bottom: 90, width: 260, height: 260, borderRadius: 160, opacity: 0.95 },
