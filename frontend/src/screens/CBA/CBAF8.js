// frontend/src/screens/CBA/CBAF8.js
// CBAF8 – Tuition / fees screen for CBA (gold theme, with remote CBA fees)
...
// 🆕 use same name source as CCSF8
import { getCurrentVisitorName } from "../../state/userSession";
// 🆕 CBA fees loader from shared helper
...
export default function CBAF8({ navigation }) {
  ...
  // 🧑‍🎓 Name default from FillupScreen shared session
  const initialName = getCurrentVisitorName() || "Juan Dela Cruz";
  ...
  // 🆕 load CBA fee config from Supabase (via helper)
  useEffect(() => {
    (async () => {
      try {
        const remote = await loadDeptFees("CAS");
        console.log("[CBAF8] remote CBA fees:", remote);
        if (!remote) return;

        // keys should match what AdminScreen / Supabase saves:
        ...
        console.log("[CBAF8] failed to load remote CBA fees:", err);
      }
    })();
  }, []);
  ...
  return (
    <SafeAreaView style={s.screen}>
      ...
      {/* CBA gold warm background */}
      <LinearGradient colors={["#fff9f0", "#fff6ee"]} style={s.bg} />

      <View style={s.layerTopRight} />
      <View style={s.layerBottomLeft} />

      {/* BACK → CBAF4 (same hub as in CBAF4 top card) */}
