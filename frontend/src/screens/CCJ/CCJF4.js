// frontend/src/screens/CCJ/CCJF4.js
// CCJF4 — gray/black gradient cards + shimmer + subtle glow + 3D press/tilt
// Theme: gray / black (based on CCSF4 layout/behavior)
...
// card images
const IMG_TUITION = require("../../../assets/tuition.png");   // top → CCJF8
const IMG_UNIFORM = require("../../../assets/uniform.png");   // left → CCJF10
const IMG_FAQ = require("../../../assets/faq.png");           // right → CCJF9
...
export default function CCJF4({ navigation }) {
  ...
  function makeResponder(i) {
    return PanResponder.create({
      ...
      onPanResponderRelease: () => {
        Animated.parallel([
          Animated.spring(tilt[i].x, {
            toValue: 0,
            friction: 6,
            useNativeDriver: false,
          }),
          Animated.spring(tilt[i].y, {
            toValue: 0,
            friction: 6,
            useNativeDriver: false,
          }),
  ...
  // press handler for stronger glow + navigation
  ...
  return (
    <SafeAreaView style={s.screen}>
      ...
      {/* Back → CCJF3 (same pattern as CCSF4 → CCSF3) */}
      <TouchableOpacity
        style={s.back}
        onPress={() => navSafe("CCJF3")}
        activeOpacity={0.85}
      >
      ...
      <View style={s.centerContainer}>
        {/* TOP card (TUITION → CCJF8) */}
        <Animated.View
          {...responders[0].panHandlers}
          style={[s.topWrapper, cardAnimatedStyle(0)]}
        >
          <TouchableWithoutFeedback onPress={() => handlePress(0, "CCJF8")}>
        ...
        {/* BOTTOM row: left (UNIFORM → CCJF10) and right (FAQ → CCJF9) */}
