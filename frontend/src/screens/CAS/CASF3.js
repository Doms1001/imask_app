// CASF3.js (Rethemed to Violet / Purple)
...
export default function CASF3({ navigation }) {
  const dummy = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    const a = Animated.timing(dummy, { toValue: 1, duration: 600, useNativeDriver: true });
    a.start();
  ...
  useEffect(() => {
    ...
    const enter = Animated.stagger(110, seq);
    enter.start();
  ...
  function makeResponder(i) {
    return PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        const rx = Math.max(-18, Math.min(18, -gestureState.dx / 8));
        const ry = Math.max(-12, Math.min(12, gestureState.dy / 10));
    ...
    <View style={m.container}>
      <View style={m.leftColumn}>
        ...
        <Animated.View {...responders[0].panHandlers} style={[m.cardWrapper, cardAnimatedStyle(0)]}>
          <Animated.View style={[m.card, m.gradientCardTop]}>
            ...
            <TouchableWithoutFeedback onPress={() => handlePress(0, 'CASF5')}>
              <View style={m.centered}>
                <Image source={IMG_NEWS} style={m.bigImage} />
              </View>
            </TouchableWithoutFeedback>
            <Animated.View style={[m.cardGlow, { opacity: glowOpac[0], backgroundColor: 'rgba(140,70,220,0.18)' }]} />
          </Animated.View>
        </Animated.View>
        ...
        <Animated.View {...responders[1].panHandlers} style={[m.cardWrapper, cardAnimatedStyle(1)]}>
          <Animated.View style={[m.card, m.gradientCardSmall]}>
            ...
            <TouchableWithoutFeedback onPress={() => handlePress(1, 'CASF7')}>
              <View style={m.centered}>
                <Image source={IMG_ANNOUNCE} style={m.smallImage} />
              </View>
            </TouchableWithoutFeedback>
            <Animated.View style={[m.cardGlow, { opacity: glowOpac[1], backgroundColor: 'rgba(160,100,230,0.16)' }]} />
          </Animated.View>
        </Animated.View>
      </View>
      ...
      <Animated.View {...responders[2].panHandlers} style={[m.cardWrapper, cardAnimatedStyle(2)]}>
        <Animated.View style={[m.card, m.gradientCardRight]}>
          ...
          <TouchableWithoutFeedback onPress={() => handlePress(2, 'CASF6')}>
            <View style={m.centered}>
              <Image source={IMG_EVENT} style={m.eventImage} />
            </View>
          </TouchableWithoutFeedback>
          <Animated.View style={[m.cardGlow, { opacity: glowOpac[2], backgroundColor: 'rgba(140,80,220,0.18)' }]} />
        </Animated.View>
      </Animated.View>
    </View>
  );
  ...
  return (
    <SafeAreaView style={s.screen}>
      ...
      <TouchableWithoutFeedback onPress={() => navSafe('CASF2')}>
        <View style={s.back}>
      ...
      <BottomPager navigation={navigation} activeIndex={1} targets={['CASF2', 'CASF3', 'CASF4']} />
    </SafeAreaView>
  );
}
...
const s = StyleSheet.create({
  ...
  layerTopRight: { position: 'absolute', right: -40, top: -20, width: 220, height: 220, backgroundColor: '#7b2cbf', borderRadius: 18, opacity: 0.14 },
  layerBottomLeft: { position: 'absolute', left: -60, bottom: -80, width: 260, height: 260, backgroundColor: '#c9a6ff', borderRadius: 160, opacity: 0.28 },
...
const m = StyleSheet.create({
  ...
  cardGlow: { position: 'absolute', bottom: -8, width: 160, height: 22, borderRadius: 12, backgroundColor: 'rgba(160,100,230,0.16)' },
