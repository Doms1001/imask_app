// frontend/src/screens/COA/COAF8.js
// COAF8 – Tuition / fees screen for COA, with Name coming from FillupScreen
// and fees loaded from COA Supabase config
...
import { loadDeptFees } from "../../lib/ccsMediaHelpers";         // 👈 COA version

const { width, height } = Dimensions.get("window");
const BACK = require("../../../assets/back.png");
const GMAIL_IMG = require("../../../assets/gmail.png");

export default function COAF8({ navigation }) {
  ...
  // Name default from FillupScreen via shared state
  ...
  // 🔽 load remote COA config from Supabase
  useEffect(() => {
    (async () => {
      try {
        const remote = await loadDeptFees("CAS");
        console.log("[COAF8] remote COA fees:", remote);
        if (!remote) return;
        ...
        if (remote.down) setDownPayment(remote.down); // key "down" in payload
      } catch (err) {
        console.log("[COAF8] failed to load remote fees:", err);
      }
    })();
  }, []);
